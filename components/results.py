...
class Results:
    ...
    def compute_results(self) -> None:
        ...
        #     histogram_data = self.df[~self.df.id.isin(get_sus_ids())].wave // bin_width * bin_width
        ...
        if self.show_hist:
            to_be_displayed_styler = self.show_hist_preparation(to_be_displayed, self.df, date)
        else:
            self._styler()
            to_be_displayed_styler = self.regular_preparation(to_be_displayed, self.df)
