...
def compute_winners(df, options=None):
    ...
    how_many = how_col.slider(
        "How many past tournaments?", min_value=1, max_value=len(df[df.position == 1]), value=len(df[df.position == 1])
    )
    ...
    if scoring == "Only winners":
        ...
        colormap = st.selectbox(
            ...
            [
                item
                for item in dir(px.colors.sequential)
                if not item.startswith("__") and not item.startswith("swatches")
            ],
        )
    ...
    fig = px.pie(
        graph_df, values="count", names="name", title="Winners of champ, courtesy of Jim", **additional_options
    )
